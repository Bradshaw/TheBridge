...
function fudge_mt:countToAnimation(piecename, number, options)
	local options = options or {}
	local numlen = options.numlen or (""..number):len()
	local animation = {}
	for i=(options.startAt or 1),number do
		table.insert(animation, piecename..string.format("%0"..numlen.."d", i))
	end
	self:animate((options.name or piecename.."_ani"), animation, options)
end
